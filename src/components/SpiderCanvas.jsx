...
const SpiderCanvas = ({ containerRef }) => {
  ...
  useEffect(() => {
    ...
    // Use Intersection Observer to check if the section is visible
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsActive(entry.isIntersecting);
      },
      { threshold: 0.1 } // 10% visible triggers active
    );
  ...
  useEffect(() => {
    ...
    function anim(t) {
      if (!isActive) return; // stop animation if not active
      const container = containerRef?.current || document.body;
      if (w !== container.clientWidth) w = (canvas.width = container.clientWidth);
      if (h !== container.clientHeight) h = (canvas.height = container.clientHeight);
      ctx.fillStyle = "#000";
      drawCircle(0, 0, w * 10);
    ...
    if (isActive) animRef.current = requestAnimationFrame(anim);
